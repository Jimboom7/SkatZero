...
model_path = 'models/checkpoints/skat_resnet_D/model.tar'
model_path_2 = 'models/checkpoints/skat_resnet_new_D/model.tar'
model_path_new = 'models/checkpoints/skat_resnet_new_D/model_new.tar'
...
for i in range(0,3):
    new_weights = torch.cat((torch.zeros((512, 512), device='cuda:0'), all_models['model_state_dict'][i]['fc_layers.0.weight'][:,:]), dim=1) # second number: zeros to add
    ...
    state = all_models['optimizer_state_dict'][i]['state'][16]
    new_opt = torch.cat((torch.zeros((512, 512), device='cuda:0') + state['square_avg'].mean(), state['square_avg'][:,:]), dim=1)
    all_models_2['optimizer_state_dict'][i]['state'][16]['square_avg'] = new_opt
    for j in range(17, 28):
        all_models_2['optimizer_state_dict'][i]['state'][j]['square_avg'] = all_models['optimizer_state_dict'][i]['state'][j]['square_avg']
