...
model_path = 'models/checkpoints/skat_resnet_new_D/model.tar'
model_path_2 = 'models/checkpoints/skat_resnet_new_2_D/model_original.tar'
model_path_new = 'models/checkpoints/skat_resnet_new_2_D/model_new.tar'
...
for i in range(0,3):
    new_weights = all_models['model_state_dict'][i]['fc_layers.0.weight'][:,:]
    ...
    # new_opt = state['square_avg'][:,:]
    # all_models_2['optimizer_state_dict'][i]['state'][16]['square_avg'] = new_opt
    # for j in range(21, 32):
    #     all_models_2['optimizer_state_dict'][i]['state'][j]['square_avg'] = all_models['optimizer_state_dict'][i]['state'][j - 4]['square_avg']
