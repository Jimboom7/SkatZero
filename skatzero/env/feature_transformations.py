...
def cards2array(cards):
    ...
    for c in cards:
        matrix[CardSuit2Column[c[0]], Card2Column[c[1]]] = 1
    return matrix.flatten()
...
def action_seq_list2array(action_seq_list):
    ...
    for row, card in enumerate(action_seq_list):
        action_seq_array[row, :] = cards2array([card])
    ...
    for player, card in trace:
        ...
        if trick_counter % 3 == 0:
            ...
            if base_card[1] == 'J' or base_card[0] == trump:
                ...
                matrix[CardSuit2Column[base_card[0]], :7] = 1
    return matrix.flatten()
...
def _get_obs_common(infoset):
    ...
    trick1 = cards2array(None)
    trick2 = cards2array(None)
    if len(infoset.trick) >= 1:
        trick1 = cards2array([infoset.trick[0][1]])
    if len(infoset.trick) == 2:
        trick2 = cards2array([infoset.trick[1][1]])
    ...
    z = action_seq_list2array(process_action_seq(infoset.card_play_action_seq))
    ...
    return (num_legal_actions, my_handcards, my_handcards_batch, other_handcards, other_handcards_batch, trick1, trick1_batch, trick2, trick2_batch,
            my_action_batch, soloplayer_points, soloplayer_points_batch, opponent_points, opponent_points_batch, z, z_batch)
...
def _get_obs_soloplayer(infoset):
    ...
    my_action_batch, soloplayer_points, soloplayer_points_batch, opponent_points, opponent_points_batch, z, z_batch) = _get_obs_common(infoset)

    skat = cards2array(infoset.skat_cards)
    missing_cards_left = calculate_missing_cards('opponent_left', infoset.card_play_action_seq, infoset.trump)
    missing_cards_right = calculate_missing_cards('opponent_right', infoset.card_play_action_seq, infoset.trump)
    opponent_left_played_cards = cards2array(infoset.played_cards['opponent_left'])
    opponent_right_played_cards = cards2array(infoset.played_cards['opponent_right'])
...
def _get_obs_opponent(infoset):
    ...
    last_soloplayer_action = cards2array([infoset.last_move_dict['soloplayer']])
    last_teammate_action = cards2array([infoset.last_move_dict[teammate]])
    soloplayer_played_cards = cards2array(infoset.played_cards['soloplayer'])
    teammate_played_cards = cards2array(infoset.played_cards[teammate])
    missing_cards_soloplayer = calculate_missing_cards('soloplayer', infoset.card_play_action_seq, infoset.trump)
    missing_cards_teammate = calculate_missing_cards(teammate, infoset.card_play_action_seq, infoset.trump)
